...
if __name__ == '__main__':
    ...
    # --- Plots ---
    plt.figure()
    h0 = plt.hist2d(tot_all, peak_all, bins=(200,100), cmap='gnuplot2', norm=colors.LogNorm())
    plt.colorbar(h0[3])
    plt.xlabel('Time Over Threshold [nsec]')
    plt.ylabel('Peak Voltage [mV]')
    plt.minorticks_on()
    plt.savefig('tot_vs_peak_heatmap_all.pdf')

    print('Plot 1/3 done')

    plt.figure()
    h1 = plt.hist2d(tot_a, peak_a,  bins =(200, 100), cmap='gnuplot2', norm=colors.LogNorm())
    plt.colorbar(h1[3])
    plt.xlabel('Time Over Threshold [nsec]')
    plt.ylabel('Peak Voltage [mV]')
    plt.minorticks_on()
    plt.savefig('tot_vs_peak_heatmap_a_side.pdf')

    print('Plot 2/3 done')

    plt.figure()
    h2 = plt.hist2d(tot_b, peak_b, bins = (200,100), cmap='gnuplot2', norm=colors.LogNorm())
    plt.colorbar(h2[3])
    plt.xlabel('Time Over Threshold [nsec]')
    plt.ylabel('Peak Voltage [mV]')
    plt.minorticks_on()
    plt.savefig('tot_vs_peak_heatmap_b_side.pdf')
    ...
    np.savetxt("peak_all.txt", peak_all)
    np.savetxt("peak_a.txt", peak_a)
    np.savetxt("peak_b.txt", peak_b)
