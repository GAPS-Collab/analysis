...
if __name__ == '__main__':
    ...
    for f in tqdm(tof_files, desc = 'reading raw .tof.gaps files'):
        ...
        for pack in reader:
            ...
            status = tof_ev.mastertriggerevent.status
            if int(status) != 0:
                if int(status) != 23:
                    print(int(status))
            if int(status) == 16: continue
                ...
                try:
                    ...
                    for waveform in tof_ev.waveforms:
                        if waveform.rb_id == rb and waveform.rb_channel_a == ch:
                            ...
                            if np.max(voltages) > 400 and np.max(voltages) < 700:
                                aligned_wf = align_wf(voltages, 512)
    ...
    plt.xlim(-50, 200)
    plt.savefig('no16_avg_wf.pdf')
    
    x_shifted = x-np.argmax(wfs)
    mask = (x_shifted >= -50) & (x_shifted <= 250)

    data = np.column_stack((x_shifted[mask], wfs[mask]))
    np.savetxt('no16_average_waveform.txt', data, fmt="%.6f", header="x y")
    
    plt.figure()
    plt.hist(fwhm_vals, histtype='step', bins=60)
    plt.xlim(0,50)
    plt.minorticks_on()
    plt.savefig('no_16fwhm_dist_650_725.pdf')
