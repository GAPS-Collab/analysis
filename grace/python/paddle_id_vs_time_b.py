        ...
        for pack in reader:
            ...
            for x in range(len(tof_ev.hits)):
                try:
                    ...
                    time_b.append(time_b_side)
                except Exception as e:
                    print(f"Error at hit {x}: {e}")
                    continue


    plt.hist2d(time_b, paddle_ids, bins=[len(set(paddle_ids)), 100], cmap="plasma", norm=colors.LogNorm())
    plt.colorbar(label="Counts")
    plt.ylabel("Paddle ID")
    plt.xlabel("Time_B [nsec]")
    plt.yticks(np.arange(0,161,10))
    plt.xlim(0,550)
    plt.savefig("paddle_id_vs_time_b.pdf")
    plt.show()
